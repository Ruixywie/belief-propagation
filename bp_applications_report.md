...
>
> - **算法 | Algorithm**：Min-Sum Loopy BP（对数域 Max-Product BP）/ Min-Sum Loopy BP (log-domain Max-Product BP)
> - **图模型 | Graphical Model**：4-连接网格 MRF，像素为变量节点 / 4-connected grid MRF with pixels as variable nodes
> - **一元势 | Unary**：高斯似然 $(v(l) - y)^2 / 2\sigma^2$ / Gaussian likelihood
> - **二元势 | Pairwise**：截断二次/线性模型（边缘保留）/ Truncated quadratic/linear model (edge-preserving)
> - **关键技术 | Key Techniques**：距离变换加速 + 阻尼消息更新 + float32 优化 / Distance transform acceleration + damped message updates + float32 optimization
> - **推理目标 | Inference Goal**：MAP 估计（能量最小化）/ MAP estimation (energy minimization)
...
> **技术总结 | Technical Summary**
>
> - **算法 | Algorithm**：Hierarchical BP（分层置信传播）— 多尺度粗到细 Min-Sum BP / Hierarchical BP — multi-scale coarse-to-fine Min-Sum BP
> - **图模型 | Graphical Model**：4-连接网格 MRF，像素为变量节点，视差为标签 / 4-connected grid MRF with pixels as variable nodes, disparity as labels
> - **数据代价 | Data Cost**：Census Transform 汉明距离 / SAD 窗口匹配 / Census Transform Hamming distance / SAD window matching
> - **平滑代价 | Smoothness Cost**：截断二次距离变换 / Truncated quadratic distance transform
> - **关键技术 | Key Techniques**：高斯金字塔 + 消息上采样（标签维线性插值）+ 阻尼 + 后处理（中值/双边滤波）+ CuPy GPU 加速 / Gaussian pyramid + message upsampling (label-dim linear interpolation) + damping + post-processing (median/bilateral filter) + CuPy GPU acceleration
> - **推理目标 | Inference Goal**：MAP 估计（MRF 能量最小化 → 最优视差图）/ MAP estimation (MRF energy minimization → optimal disparity map)
...
> **技术总结 | Technical Summary**
>
> - **算法 | Algorithm**：SCWRL4（Dead-End Elimination + 图分解）/ TRBP（Tree-Reweighted BP）/ SCWRL4 (DEE + graph decomposition) / TRBP (Tree-Reweighted BP)
> - **图模型 | Graphical Model**：残基交互图（空间距离 < 阈值的残基对连边），变量 = rotamer 选择 / Residue interaction graph (edges between residue pairs within distance threshold), variables = rotamer choice
> - **一元势 | Unary**：残基自身能量（主链-侧链相互作用 + rotamer 先验）/ Residue self-energy (backbone-sidechain interaction + rotamer prior)
> - **二元势 | Pairwise**：残基对相互作用（范德华力 + 静电）/ Residue pair interaction (van der Waals + electrostatics)
> - **关键技术 | Key Techniques**：DEE 剪枝 + 图连通分量分解 + ROSETTA 力场 + TRBP 生成树凸组合 / DEE pruning + graph connected component decomposition + ROSETTA force field + TRBP spanning tree convex combination
> - **推理目标 | Inference Goal**：MAP 估计（全局能量最小的 rotamer 组合）/ MAP estimation (globally minimal energy rotamer assignment)
